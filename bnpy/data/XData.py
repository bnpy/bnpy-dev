...
from bnpy.utils_array import as1D, as2D, toCArray
from bnpy.utils_parallel import \
    convert_shared_mem_arr_to_numpy_arr, \
    convert_numpy_arr_to_shared_mem_arr
...
class XData(object):
    ...
    def to_shared_mem_dict(self):
        ''' Create dict with copies of raw data as shared memory arrays

        Returns
        -------
        dict : each field is a shared memory array or primitive type
        '''
        shared_mem_dict = dict()
        shared_mem_dict['X'] = convert_numpy_arr_to_shared_mem_arr(self.X)
        shared_mem_dict['nObsTotal'] = int(self.nObsTotal)
        if hasattr(self, 'Xprev'):
            shared_mem_dict['Xprev'] = \
                convert_numpy_arr_to_shared_mem_arr(self.Xprev)
        if hasattr(self, 'Y'):
            shared_mem_dict['Y'] = \
                convert_numpy_arr_to_shared_mem_arr(self.Y)
        return shared_mem_dict

    def get_function_to_make_dataset_from_shared_mem_dict(self):
        ...
        f : function handle
        """
        return make_dataset_from_shared_mem_dict
...
def make_dataset_from_shared_mem_dict(
        shared_mem_dict,
        slice_interval=(0, None),
        batchID=None):
    ...
    >>> data_slice = make_dataset_from_shared_mem_dict(shared_mem_dict)
    >>> np.allclose(dataset.X, data_slice.X)
    True
    >>> np.allclose(dataset.nObs, data_slice.nObs)
    True
    >>> dataset.dim == data_slice.dim
    True
    >>> a_slice = make_dataset_from_shared_mem_dict(shared_mem_dict, (0, 2))
    >>> a_slice.nObs
    2
    """
    if batchID is not None and batchID in shared_mem_dict:
        shared_mem_dict = shared_mem_dict[batchID]

    # Make local views (NOT copies) to shared mem arrays
    if isinstance(slice_interval, slice):
        cslice = slice_interval
    else:
        if slice_interval is None:
            cslice = slice(0, None)
        elif isinstance(slice_interval, tuple):
            assert len(slice_interval) == 2
            cslice = slice(slice_interval[0], slice_interval[1])

    X = convert_shared_mem_arr_to_numpy_arr(shared_mem_dict['X'])[cslice]
    nObsTotal = int(shared_mem_dict['nObsTotal'])

    # Create sliced arrays and return
    if 'Xprev' in shared_mem_dict:
        Xprev = convert_shared_mem_arr_to_numpy_arr(
            shared_mem_dict['Xprev'])[cslice]
    else:
        Xprev = None
    if 'Y' in shared_mem_dict:
        Y = convert_shared_mem_arr_to_numpy_arr(
            shared_mem_dict['Y'])[cslice]
    else:
        Y = None
    return XData(
        X=X,
